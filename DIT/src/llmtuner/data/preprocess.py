    ...
    def preprocess_supervised_dataset(examples: Dict[str, List[Any]]) -> Dict[str, List[List[int]]]:
        ...
        for query, response, history, system in construct_example(examples):
            ...
            for turn_idx, (source_ids, target_ids) in enumerate(template.encode_multiturn(
                ...
                # (prefix_ids(bos_ids) + query_ids, resp_ids + eos_ids)
                ...
                # 对截断后的内容要加：template和<s>
                if source_len > max_source_len:
                    # print('#########################33')
                    # print(source_ids[:max_source_len])
                    # "\nNow the paper ends.[/INST]"
                    source_ids = source_ids[:max_source_len-10] + [29871,13,10454,278,5650,10614,7226,29914,25580,29962]
                    # print(source_ids)
                if target_len > max_target_len:
                    # print('#########################22')
                    # print(target_ids[:max_target_len])
                    target_ids = target_ids[:max_target_len-1] + [tokenizer.eos_token_id]
                    # print(target_ids)
            ...
            if len(input_ids) > data_args.cutoff_len:
                input_ids = input_ids[:data_args.cutoff_len-1] + [tokenizer.eos_token_id]
                labels = labels[:data_args.cutoff_len-1] + [tokenizer.eos_token_id]
