...
class GenerationMixin:
    ...
    def _get_generation_mode(
        ...
        this is very useful to know about what hyperparameter changes the generation mode
        """
    ...
    def generate(
        ...
        **kwargs,
        ...
        print("This is the overload of origin huggingface generation function!!!!!!!")
        ...
        # we need to insert null_inputs_model_kwargs here
        ...
        # for encoder-decoder models, inputs_tensor is batched_input, to be different from later input_ids
        ...
        # 4.1 We also need to define the model kwargs for null_input models
        if null_inputs_model_kwargs.get("attention_mask", None) is None and requires_attention_mask and accepts_attention_mask:
            null_inputs_model_kwargs["attention_mask"] = self._prepare_attention_mask_for_generation(
                null_inputs_tensor, generation_config.pad_token_id, generation_config.eos_token_id
            )
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            """
            this step creates the encoder_outputs object to be used for encoder-decoder model for conditional generation, if we want to add context-aware decoding, we can add here
            """
        ...
        # if encoder_decoder model, input_ids is batch_size * decoder_start_token_id, else inputs_tensor
        if self.config.is_encoder_decoder:
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # TODO: come up with a way to reuse cache maybe
            ...
            next_tokens_scores = logits_processor(input_ids, next_token_logits)  # basically, the decoding continues in this while loop until stopping criteria is met and the while loop is break
            ...
            # update generated ids, model inputs, and length for next step
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            # correspondingly update null_inputs to include the newly generated tokens
            if context_aware_decoding_alpha > 0.:
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs) # dictionary {"input_ids": torch.LongTensor, "attention_mask": torch.LongTensor, "past_key_values": None, "use_cache": bool}
            null_model_inputs = self.prepare_inputs_for_generation(null_input_ids, **null_inputs_model_kwargs) if context_aware_decoding_alpha > 0. else None
            ...
            if probs.isnan().any():
                probs = torch.nan_to_num(probs, 1e-10)
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if context_aware_decoding_alpha > 0.:
                ...
                if synced_gpus and this_peer_finished:
                    continue  # don't waste resources running the code we don't need
                null_next_token_logits = null_outputs.logits[:, -1, :]  # here the shape is (bz, seq_len)
                next_token_logits = next_token_logits * (context_aware_decoding_alpha+1.) - null_next_token_logits * context_aware_decoding_alpha
    ...
    def beam_sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # print(f"null model inputs -> {null_model_inputs['input_ids']}")
